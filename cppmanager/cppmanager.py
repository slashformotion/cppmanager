...
@click.option(
    ...
    "-l",
    "language",
    type=click.Choice(["c", "cpp"], case_sensitive=False),
    default="cpp",
)
@click.pass_context
    ...
    with click.progressbar(length=100) as bar:
        ...
        if main_name in [path.stem for path in sources_paths]:
            raise click.BadParameter(
                f"The file {main_name}.{sources_suffix} already exists and you want to create one more, that's not possible buddy"
            )
        ...
        for header_name in headers_names:
            ...
            if filename in [path.stem for path in headers_paths]:
                raise click.BadParameter(
                    f"The file {filename}.{headers_suffix} already exists and you want to create one more, that's not possible buddy"
                )

        main = Main_File(
            main_name,
            main_suffix=sources_suffix,
            header_suffix=headers_suffix,
        )
        bar.update(50)
        modules = []
        for header_name in headers_names:
            module = Module(
                header_name, module_suffix=headers_suffix, main_suffix=sources_suffix
            )
...
def add(ctx, main_name, headers_names):
    with click.progressbar(length=100) as bar:
        ...
        if not main_name in [path.stem for path in sources_paths]:
            raise click.BadParameter(
                f"The file {main_name}.{sources_suffix} don't exists, it should to use the 'add' mode."
            )
        ...
        main = Main_File(
            main_name, main_suffix=sources_suffix, header_suffix=headers_suffix
        )
        bar.update(50)
        modules = []
        for header_name in headers_names:
